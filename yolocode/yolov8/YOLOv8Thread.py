...
class YOLOv8Thread(QThread):
    ...
    def __init__(self):
        ...
        self.labels_dict = {}  # 当前图片的检测结果
        self.all_labels_dict = {}  # 添加：存储所有图片的检测结果汇总
    ...
    def detect(self, is_folder_last=False):
        ...
        while True:
            if self.stop_dtc:
                ...
                self.send_result_picture.emit(self.all_labels_dict)  # 发送汇总图片结果
                for key, value in self.all_labels_dict.items():
                    self.results_table.append([key, str(value)])
                self.all_labels_dict = {}  # 重置汇总字典
            ...
            if self.is_continue:
                ...
                for i in range(n):
                    ...
                    # 累加到 all_labels_dict
                    for label, num in self.labels_dict.items():
                        if label in self.all_labels_dict:
                            self.all_labels_dict[label] += num
                        else:
                            self.all_labels_dict[label] = num
                    ...
                    if self.save_res:
                        save_path = str(self.save_path / p.name)
