...
KEYS_LEFT_BOX_MENU = ['src_setting', 'src_webcam', 'src_folder', 'src_camera', 'src_vsmode', 'src_setting']
...
class YOLOSHOW(QMainWindow, YOLOSHOWBASE):
    def __init__(self):
        ...
        specified_model = "yolo11n-twobackbone.pt"  # 替换为你的目标文件名
        if self.pt_list: 
            if specified_model in self.pt_list:
                index = self.pt_list.index(specified_model)
            else:
                index = 0
            self.ui.model_box.setCurrentIndex(index)
        ...
        self.ui.src_vsmode.clicked.connect(self.openChatDialog)
